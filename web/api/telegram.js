...
Inspect these secondhand golf-club listing photos. A conventional complete set is NOT required. Irons-only and partial sets are desirable at the right price, and a bag with a few usable irons plus a putter can beat a pricier incomplete one. Entry-level boxed sets (Callaway Strata, Wilson, Top Flite) are acceptable. Old clubs are acceptable - clubs do not expire, and 15-25 year old irons from a real manufacturer play fine for a beginner. A missing wedge or a generic fairway wood is a minor, cheaply-replaced gap, not a dealbreaker. Judge it as: can a right-handed adult beginner play this, at or under $300 landed, AND is the asking price clearly BELOW what the clubs are actually worth?
...
const renderAnalysis = (category, d, title, price) => {
  ...
  if (category === "watches") {
    ...
    if (d.is_playable_first_set) lines.push(`
✅ Playable as a first set`);
    else lines.push(`
⚠️ Not playable as a first set`);
    if (d.is_starter_kit_quality) lines.push(`
ℹ️ Entry-level/boxed-set quality (fine for a first set)`);
    if (d.damage_found) lines.push(`\n🩹 Damage: ${d.damage_desc || "see summary"}`);
    const retail = money(d.estimated_retail_price);
    if (retail) lines.push(`\n🏷️ Retail (new): ${retail}`);
    const resale = money(d.estimated_resale_value);
