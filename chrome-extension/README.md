...
Deal Scout replaces the unreliable Facebook proxy browser with your real, logged-in Chrome session. About every 10 minutes it fetches each enabled saved search in the background using your logged-in Facebook cookies (no tab is opened for a Facebook target), extracts listing candidates, and sends them to `/api/scout-ingest`. Only a `generic-og` target still opens a real tab, in a dedicated always-minimized scan window. The Python bot later consumes those candidates through its existing scoring, AI-check, dedupe, and alert pipeline.
...
`generic-og` is intentionally minimal: it returns the loaded page's canonical URL and OG fields rather than trying to understand a site's search-result schema. It is useful when a target URL itself represents one listing and exposes a numeric `product:price:amount`. A true multi-result site needs a new parser file under `content-scripts/`, a parser option in `popup.html`, and a dispatch entry in `background.js`; that is code, not configuration. Facebook does not use a content script at all: its useful data is in embedded `application/json` blocks in the server-rendered HTML, so `background.js` fetches the page directly and parses it in `extractFacebookListingsFromHtml()`.
...
- Click **Scan Now**. No Facebook tab should open at all; watch the service-worker console for the per-target result.
...
- Disable a target and scan again; verify the popup no longer reports a result for it.
