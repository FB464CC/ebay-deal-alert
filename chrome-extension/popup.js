...
async function requestUrlPermission(rawUrl) {
...
document.getElementById("setupForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  const ingestEndpoint = document.getElementById("endpoint").value.trim();
  if (!await requestUrlPermission(ingestEndpoint)) return void (statusNode.textContent = "Endpoint permission is required to send listings.");
